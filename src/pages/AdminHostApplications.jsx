    ...
    <div className="space-y-6">
      ...
      {selected && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => !busy && setSelected(null)}><div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-[2rem] bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-7 p-6">
          ...
          <div><h3 className="font-bold text-[#0B0B45]">Experience</h3><p className="mt-2 rounded-3xl bg-[#F3F4F6] p-4 text-sm text-[#1f2937] whitespace-pre-wrap">{selected.experience || 'No additional notes.'}</p></div>
...
function Detail({ label, value }) { return <div className="rounded-2xl bg-[#F3F4F6] p-3"><p className="text-xs text-[#6b7280]">{label}</p><p className="mt-1 text-sm font-semibold text-[#1f2937] break-words">{value === null || value === undefined || value === '' ? '—' : String(value)}</p></div>; }
