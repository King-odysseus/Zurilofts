...
function AdminHostApplications() {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? <div className="py-16 text-center text-[#6b7280]">Loading applications...</div> : applications.length === 0 ? <div className="rounded-3xl shadow-sm bg-white p-12 text-center text-[#6b7280]">No applications in this view.</div> : (
...
function Detail({ label, value }) { return <div className="rounded-2xl bg-canvas p-3"><p className="text-xs text-[#6b7280]">{label}</p><p className="mt-1 text-sm font-semibold text-[#1f2937] break-words">{value === null || value === undefined || value === '' ? '—' : String(value)}</p></div>; }
