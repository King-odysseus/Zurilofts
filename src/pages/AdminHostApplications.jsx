...
function AdminHostApplications() {
  ...
  return (
    <div className="space-y-6">
      ...
      {selected && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => !busy && setSelected(null)}><div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-[14px] bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-[#E5E7EB] bg-white px-6 py-5"><div><h2 className="text-xl font-bold text-[#222222]">{selected.legalName}</h2><p className="text-sm text-[#6b7280]">{selected.contactEmail}</p></div><button onClick={() => setSelected(null)} className="rounded-full px-3 py-2 text-xl text-[#6b7280] hover:bg-gray-100">&times;</button></div>
