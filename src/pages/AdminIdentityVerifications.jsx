    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4 border-b border-[#E3E8EF] pb-6">
        ...
        <select
          ...
          className="min-h-[40px] rounded-lg border border-[#E3E8EF] bg-white px-4 py-2 text-sm text-[#0B1F42] focus:outline-none focus:border-[#C49A6C] focus:ring-2 focus:ring-[#C49A6C]/20"
        >
      ...
      >
        {statusOptions.map((option) => (
          <button
            ...
            className={`rounded-full px-4 py-2 text-xs font-semibold transition-colors ${status === option.value ? "bg-[#2563EB] text-white" : "border border-[#E5E7EB] bg-white text-[#222222] hover:bg-[#F7F7F5]"}`}
          >
            {option.label}
          </button>
        ))}
      ...
      {message && (
        <div className="rounded-[14px] bg-[#222222]/5 px-4 py-3 text-sm text-[#222222]">
          {message}
        </div>
      )}
      {loading ? (
        ...
        <div className="rounded-[14px] bg-white p-12 text-center text-[#6b7280] shadow-sm">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.map((v) => (
                <tr
                  key={v.id}
                  className="border-b border-[#E5E7EB]/60 hover:bg-[#222222]/5"
                >
                  <td className="p-4">
                  ...
                  <td className="p-4">
                    <button
                      onClick={() => open(v.id)}
                      disabled={busy === v.id}
                      className="rounded-lg bg-[#C49A6C] px-4 py-2 text-xs font-semibold text-white hover:bg-[#B8895C] disabled:opacity-50"
                    >
                      Review
                    </button>
                  </td>
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  {selected.documents?.map((document) => (
                    <button
                      ...
                      className="rounded-[14px] shadow-sm p-4 text-left hover:shadow-md disabled:opacity-50"
                    >
                      <p className="font-semibold text-[#222222]">
              ...
              {selected.status === "SUBMITTED" && (
                <div className="rounded-[14px] border border-[#E5E7EB] p-5">
                  ...
                  <textarea
                    ...
                    rows="3"
                    maxLength="2000"
                    className="w-full rounded-[14px] border border-[#E5E7EB] bg-white px-5 py-3 focus:outline-none focus:border-[#2563EB] focus:ring-2 focus:ring-[#2563EB]/20"
                  />
                  <div className="mt-4 flex flex-wrap gap-3">
                    <button
                      onClick={() => review("approve")}
                      disabled={Boolean(busy)}
                      className="min-h-[44px] rounded-lg bg-green-600 px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-50"
                    >
                      Approve
                    </button>
                    <button
                      ...
                      disabled={Boolean(busy)}
                      className="min-h-[44px] rounded-lg border border-red-300 px-5 py-2.5 text-sm font-semibold text-red-600 disabled:opacity-50"
                    >
                      Reject
                    </button>
                  </div>
