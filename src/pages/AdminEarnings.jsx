...
function AdminEarnings() {
  ...
  const metrics = useMemo(() => {
    ...
    const avgBookingValue =
      t.bookings > 0 ? Math.round(t.earnings / t.bookings) : 0;
    const confirmationRate =
      t.bookings > 0 ? Math.round((t.confirmedBookings / t.bookings) * 100) : 0;
    ...
    const avgPerProperty =
      activeProperties > 0 ? Math.round(t.earnings / activeProperties) : 0;
    const bedTotal = t.bed1Earnings + t.bed2Earnings;
    const bed1Share =
      bedTotal > 0 ? Math.round((t.bed1Earnings / bedTotal) * 100) : 0;
    const bed2Share = bedTotal > 0 ? 100 - bed1Share : 0;
    // Fee breakdown percentages
    const serviceFeePct =
      t.grossRent > 0 ? Math.round((t.serviceFees / t.grossRent) * 100) : 0;
    const hostNetPct =
      t.grossRent > 0 ? Math.round((t.hostNet / t.grossRent) * 100) : 0;
    ...
    return {
      ...
      bed2Share,
      topEarner:
        [...earningRows].sort((a, b) => b.earnings - a.earnings)[0] || null,
      serviceFeePct,
      hostNetPct,
      whtPct,
    };
  ...
  function handleExportPDF() {
    ...
    const summaryData = [
      [
        "Active Bookings (ex. cancelled)",
        String(filteredTotals.bookings.toLocaleString()),
      ],
      [
        "Confirmed Bookings",
        String(filteredTotals.confirmedBookings.toLocaleString()),
      ],
      ["Active Earnings (KES)", filteredTotals.earnings.toLocaleString()],
      [
        "Confirmed Earnings (KES)",
        filteredTotals.confirmedEarnings.toLocaleString(),
      ],
    ];
    ...
    const feeData = [
      ...
      [
        "Service Fees (12% of subtotal)",
        `KES ${filteredTotals.serviceFees.toLocaleString()}`,
        `${metrics.serviceFeePct}% of gross`,
      ],
      [
        "Discounts",
        `-KES ${filteredTotals.discounts.toLocaleString()}`,
        "Promo code discounts",
      ],
      [
        "Host Net Earnings",
        `KES ${filteredTotals.hostNet.toLocaleString()}`,
        `${metrics.hostNetPct}% of gross`,
      ],
      [
        "Withholding Tax (WHT 5%)",
        `KES ${filteredTotals.wht.toLocaleString()}`,
        `~${metrics.whtPct}% of host net`,
      ],
    ];
  ...
  function handleExportCSV() {
    ...
    csv +=
      "Property,Active Booked,Gross Rent (KES),Service Fee (KES),Host Net (KES),WHT (KES),Total (KES)\n";
  ...
  const cards = [
    ...
    {
      label: "Service Fees (KES)",
      value: filteredTotals.serviceFees.toLocaleString(),
      color: "bg-amber-500",
      sub: `12% of subtotal · ${metrics.serviceFeePct}% of gross`,
    },
    {
      label: "Host Net (KES)",
      value: filteredTotals.hostNet.toLocaleString(),
      color: "bg-green-600",
      sub: `After fees · ${metrics.hostNetPct}% of gross`,
    },
    {
      label: "WHT 5% (KES)",
  ...
  const feeCards = [
    {
      label: "Discounts (KES)",
      value: `${filteredTotals.discounts.toLocaleString()}`,
      sub: "Promo code deductions",
    },
    {
      label: "Confirmed Revenue (KES)",
      value: filteredTotals.confirmedEarnings.toLocaleString(),
      sub: `KES ${metrics.avgBookingValue.toLocaleString()} avg / booking`,
    },
    {
      label: "Avg / Property (KES)",
      value: metrics.avgPerProperty.toLocaleString(),
      sub: `${metrics.activeProperties} earning properties`,
    },
    {
      label: "Take-Home (KES)",
      value: Math.max(
        0,
        filteredTotals.hostNet - filteredTotals.wht,
      ).toLocaleString(),
      sub: "Host net after WHT deduction",
    },
  ];
  ...
  const insights = [
    {
      label: "Top Earner",
      value: metrics.topEarner ? metrics.topEarner.title : "-",
      hint: metrics.topEarner
        ? `KES ${metrics.topEarner.earnings.toLocaleString()}`
        : undefined,
    },
    {
      label: "Confirmation Rate",
      value: `${metrics.confirmationRate}%`,
      hint: `${filteredTotals.confirmedBookings.toLocaleString()} of ${filteredTotals.bookings.toLocaleString()} bookings`,
    },
    {
      label: "1-Bed Share",
      value: `${metrics.bed1Share}%`,
      hint: `KES ${filteredTotals.bed1Earnings.toLocaleString()}`,
    },
    {
      label: "2-Bed Share",
      value: `${metrics.bed2Share}%`,
    ...
    <div className="w-full">
      ...
      <div className="mb-6 flex flex-col gap-4 border-b border-[#E3E8EF] pb-6 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mb-1 text-2xl font-bold text-[#0B1F42]">Earnings</h1>
          <p className="text-sm text-[#5B6B82]">
            Earnings from active bookings (pending + confirmed). Cancelled
            bookings are excluded.
            {!isAdmin &&
              " View your gross rent, service fees, host net, and WHT breakdown."}
          </p>
        </div>
          ...
          {!loading && rows.length > 0 && (
            <div className="flex items-center gap-2">
              <Dropdown
                ...
                triggerClassName="min-h-[40px] px-4 py-2 rounded-lg border border-[#E3E8EF] bg-white text-[#0B1F42] font-semibold text-sm whitespace-nowrap hover:border-[#C49A6C]"
      ...
      {/* Filters */}
      <div className="mb-6 rounded-2xl border border-[#E3E8EF] bg-white p-4 shadow-[0_4px_16px_rgba(11,31,66,0.04)]">
        <div className="flex flex-col lg:flex-row gap-4">
          <div className="flex-shrink-0">
            ...
            <Dropdown
              ...
              triggerClassName="min-h-[40px] px-4 py-2 rounded-lg border border-[#E3E8EF] bg-white text-sm min-w-[160px] hover:border-[#C49A6C]"
                  ...
                  <tfoot>
                    <tr className="border-t-2 border-[#E5E7EB] bg-[#F7F7F5] font-bold text-[#222222]">
                      ...
                      <td className="py-3 px-4 text-right">
                        {filteredTotals.bookings.toLocaleString()}
                      </td>
                      <td className="py-3 px-4 text-right">
                        KES {filteredTotals.grossRent.toLocaleString()}
                      </td>
