    ...
    <article className="group relative min-w-0">
      <div className="relative aspect-[3/2] overflow-hidden rounded-2xl bg-[#E7EDF4]">
        <img
          ...
          loading="lazy"
        />
        <span className="absolute left-3 top-3 rounded-full bg-white/90 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[.08em] text-[#0B1F42] backdrop-blur-sm">
          {AREAS[item.area] || item.area}
        </span>
      </div>
      <div className="pt-3">
        <p className="text-[11px] font-semibold uppercase tracking-[.1em] text-[#B8895C]">
          {PLACE_CATEGORIES[item.category] || item.category}
        </p>
        <h2 className="mt-1.5 text-base font-semibold text-[#0B1F42]">
          {item.name}
        </h2>
        <p className="mt-2 line-clamp-3 text-xs leading-5 text-[#5B6B82]">
          {item.desc}
        </p>
        <a
          href={directionsUrl}
          ...
          className="mt-4 inline-flex min-h-10 items-center rounded-full bg-[#C49A6C] px-4 text-xs font-semibold text-white transition-colors hover:bg-[#B8895C]"
        >
          Get directions{" "}
...
function PlacesPage() {
  ...
  const filteredPlaces = useMemo(() => {
    ...
    return PLACES_TO_VISIT.filter((item) => {
      ...
      const matchesQuery =
        !normalizedQuery ||
        `${item.name} ${item.desc} ${item.area} ${item.category}`
          .toLowerCase()
          .includes(normalizedQuery);
      return matchesArea && matchesCategory && matchesQuery;
    });
  }, [area, category, query]);
  const areaOptions = Object.entries(AREAS).map(([value, label]) => ({
    value,
    label,
  }));
  const categoryOptions = Object.entries(PLACE_CATEGORIES).map(
      ...
      <main>
        <section className="mx-auto max-w-[1200px] px-4 pb-5 pt-7 md:px-8 md:pb-6 md:pt-8">
          ...
          <p className="mt-2 max-w-2xl text-sm text-[#5B6B82] md:text-base">
            Discover shopping, nature, culture, and entertainment spots near
            your stay.
          </p>
        </section>
        <section className="mx-auto max-w-[1200px] px-4 pb-6 md:px-8">
          <div className="flex min-h-14 w-full items-center rounded-full border border-[#E3E8EF] bg-white px-4 shadow-[0_8px_24px_rgba(11,31,66,0.06)] focus-within:border-[#C49A6C] focus-within:ring-2 focus-within:ring-[#C49A6C]/15">
            ...
            <input
              ...
              placeholder="Search places, neighbourhoods or experiences"
              className="discovery-search-input min-w-0 flex-1 border-0 bg-transparent py-3 text-sm text-[#0B1F42] outline-none ring-0 placeholder:text-[#94A3B8] focus:border-0 focus:outline-none focus:ring-0"
            />
            {query && (
              <button
                type="button"
                onClick={() => setQuery("")}
                className="rounded-full px-2 text-lg text-[#64748B] hover:text-[#B8895C]"
          ...
          <div className="hidden items-center justify-between gap-4 md:flex">
            <div className="flex flex-wrap gap-2">
              <Dropdown
                ...
                onChange={setArea}
                options={areaOptions}
                triggerClassName="rounded-full border border-[#E3E8EF] bg-white px-4 py-2 text-xs font-medium text-[#33415C]"
                menuClassName="left-0"
                ariaLabel="Filter places by area"
              />
              <Dropdown
                value={category}
              ...
              {hasFilters && (
                <button
                  ...
                  className="rounded-full px-3 py-2 text-xs font-semibold text-[#B8895C] hover:bg-[#C89B6D]/10"
                >
                  Clear all
                </button>
              )}
          ...
          {viewMode === "map" && filteredPlaces.length > 0 ? (
            ...
            <div className="mx-auto max-w-md rounded-2xl border border-[#E3E8EF] bg-white p-8 text-center">
              ...
              <p className="mt-2 text-sm text-[#5B6B82]">
                Try another neighbourhood or category.
              </p>
              <button
                type="button"
                onClick={clearFilters}
                className="mt-5 min-h-11 rounded-full bg-[#0B1F42] px-6 text-sm font-semibold text-white"
              >
                Clear filters
              </button>
            </div>
