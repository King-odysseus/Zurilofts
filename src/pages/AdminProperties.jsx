...
function AdminProperties() {
  ...
  async function handleReview(property, action) {
    ...
    if (action === "reject") {
      ...
      note =
        prompt(
          "Reason for suspending this listing (optional, shown to the host):",
        ) || undefined;
    }
    setSubmitting(property.id);
    try {
      const res = await apiClient.patch(
        `/admin/properties/${property.id}/review`,
        { action, note },
      );
      setProperties((prev) =>
        prev.map((p) => (p.id === property.id ? res.data.data : p)),
        ...
        <div className="flex items-center gap-3 flex-wrap">
          {isAdminView && (
            <Dropdown
              ...
              triggerClassName="min-h-[40px] px-4 rounded-lg bg-white border border-[#E3E8EF] text-sm text-[#0B1F42] focus:outline-none focus:border-[#C49A6C] focus:ring-2 focus:ring-[#C49A6C]/20"
          ...
          >
            <button
              ...
              title="Grid view"
              className={`p-2 rounded-md transition-colors ${viewMode === "grid" ? "bg-[#0B1F42] text-white" : "text-[#5B6B82] hover:text-[#0B1F42]"}`}
            >
              <svg
                className="w-4 h-4"
                fill="none"
            ...
            <button
              ...
              title="Table view"
              className={`p-2 rounded-md transition-colors ${viewMode === "table" ? "bg-[#0B1F42] text-white" : "text-[#5B6B82] hover:text-[#0B1F42]"}`}
            >
              <svg
                className="w-4 h-4"
                fill="none"
      ...
      {!isAdminView && (
        ...
        >
          {[
            ...
            [
              "IN_REVIEW",
              `In review (${properties.filter((p) => p.status === "PENDING_REVIEW").length})`,
            ],
          ].map(([value, label]) => (
            <button
              key={value || "all"}
          ...
          >
            ...
            {selectedProperty.images?.[0] && (
              <img
                ...
                alt=""
                className="mt-5 aspect-[4/3] w-full rounded-[14px] object-cover"
              />
            )}
            ...
            <div className="mt-6 flex flex-wrap gap-2 border-t border-[#E5E7EB] pt-5">
              {selectedProperty.status === "PENDING_REVIEW" && (
                <>
                  <button
                    onClick={() => handleReview(selectedProperty, "approve")}
                    disabled={submitting === selectedProperty.id}
                    className={successBtn}
                  >
                    Approve
                  </button>
                  <button
                    onClick={() => handleReview(selectedProperty, "reject")}
                    disabled={submitting === selectedProperty.id}
                    className={dangerBtn}
                  >
                    Reject
