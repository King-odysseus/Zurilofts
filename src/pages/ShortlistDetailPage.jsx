...
function SkeletonCard() {
  return (
    <div className="bg-white rounded-[14px] shadow-md overflow-hidden">
      <div className="flex flex-col sm:flex-row">
        <div className="sm:w-40 lg:w-48 h-32 sm:h-28 bg-[#E5E7EB]/40 animate-pulse" />
        <div className="flex-1 p-4 space-y-2">
          <div className="h-5 w-2/3 bg-[#E5E7EB]/40 rounded animate-pulse" />
          <div className="h-4 w-1/3 bg-[#E5E7EB]/40 rounded animate-pulse" />
          <div className="h-4 w-1/4 bg-[#E5E7EB]/40 rounded animate-pulse" />
        </div>
      </div>
    </div>
  );
...
function PropertyItem({ item, onRemove }) {
  ...
  return (
    <div className="group bg-white rounded-[14px] overflow-hidden shadow-md hover:shadow-lg transition-all duration-200">
      <div className="flex flex-col sm:flex-row">
        ...
        <div className="flex-1 p-4 flex flex-col justify-between min-w-0">
          ...
          <div className="flex items-center justify-between mt-2 pt-2 border-t border-[#E5E7EB]/30">
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-lg text-xs font-medium text-red-500 hover:text-red-700 hover:bg-red-50 transition-all duration-200"
            >
              Remove
            </button>
          </div>
...
function DeleteConfirm({ onConfirm, onCancel }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-[14px] shadow-xl p-6 mx-4 max-w-sm w-full">
        <h3 className="text-lg font-semibold text-[#222222] mb-2">Delete shortlist?</h3>
        ...
        <div className="flex gap-2 justify-end">
          <button
            type="button"
            onClick={onCancel}
            className="min-h-[44px] px-4 py-2 rounded-lg text-sm font-semibold border border-[#E5E7EB] text-[#222222] hover:bg-[#F7F7F5] transition-all duration-200"
          >
            Cancel
          </button>
          <button
            type="button"
            onClick={onConfirm}
            className="min-h-[44px] px-4 py-2 rounded-lg text-sm font-semibold bg-red-500 text-white hover:bg-red-600 transition-all duration-200"
          >
            Delete
          </button>
        </div>
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 pt-24 pb-16">
        {loading ? (
          <div className="space-y-4">
            <div className="h-8 w-48 bg-[#E5E7EB]/40 rounded animate-pulse mb-4" />
            <SkeletonCard />
            <SkeletonCard />
          </div>
        ) : error ? (
          ...
          <>
            ...
            <div className="flex items-start justify-between gap-3 mb-6">
              <div className="flex-1 min-w-0">
                {renaming ? (
                  ...
                  <h1 className="text-2xl font-bold text-[#222222] truncate">{shortlist.name}</h1>
                )}
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <button
                  onClick={handleShare}
                  className="px-3 py-1.5 rounded-lg text-xs font-semibold border border-[#E5E7EB] text-[#222222] hover:bg-[#F7F7F5] transition-all duration-200"
                >
                  {copied ? "Copied!" : "Share"}
                </button>
                <button
                  onClick={() => setRenaming(true)}
                  className="px-3 py-1.5 rounded-lg text-xs font-medium text-[#6b7280] hover:text-[#222222] hover:bg-[#E5E7EB]/30 transition-all duration-200"
                >
                  Rename
                </button>
              </div>
            ...
            {(!shortlist.items || shortlist.items.length === 0) ? (
              <div className="text-center py-16">
                ...
                <h3 className="text-lg font-semibold text-[#222222] mb-1">No saved properties</h3>
