    ...
    <article className="group relative min-w-0">
      ...
      <div className="pt-3">
        <p className="text-[11px] font-semibold uppercase tracking-[.1em] text-[#B8895C]">
          {EAT_CATEGORIES[item.category] || item.category}
        </p>
        ...
        <a
          ...
          className="mt-4 inline-flex min-h-10 items-center rounded-full bg-[#C49A6C] px-4 text-xs font-semibold text-white hover:bg-[#B8895C]"
        >
          Get directions{" "}
...
function RestaurantsPage() {
  ...
  const categoryOptions = Object.entries(EAT_CATEGORIES).map(
      ...
      <main>
        <section className="mx-auto max-w-[1200px] px-4 pb-5 pt-7 md:px-8 md:pb-6 md:pt-8">
          <p className="text-xs text-[#5B6B82]">
            Home <span className="mx-1">›</span> Restaurants
          </p>
          ...
          <p className="mt-2 max-w-2xl text-sm text-[#5B6B82] md:text-base">
            From Kenyan favourites to rooftop dining, find your next great meal.
          </p>
        </section>
        <section className="mx-auto max-w-[1200px] px-4 pb-6 md:px-8">
          <div className="flex min-h-14 w-full items-center rounded-full border border-[#E3E8EF] bg-white px-4 shadow-[0_8px_24px_rgba(11,31,66,0.06)] focus-within:border-[#C49A6C] focus-within:ring-2 focus-within:ring-[#C49A6C]/15">
            ...
            <input
              ...
              placeholder="Search restaurants, neighbourhoods or cuisines"
              className="discovery-search-input min-w-0 flex-1 border-0 bg-transparent py-3 text-sm outline-none ring-0 placeholder:text-[#94A3B8] focus:border-0 focus:outline-none focus:ring-0"
            />
            {query && (
              <button
                type="button"
                onClick={() => setQuery("")}
                className="px-2 text-lg text-[#64748B]"
          ...
          <div className="hidden items-center justify-between gap-4 md:flex">
            <div className="flex flex-wrap gap-2">
              <Dropdown
                ...
                ariaLabel="Filter restaurants by area"
              />
              <Dropdown
                value={category}
            ...
            {filtersOpen && (
              <div className="mt-3 flex flex-wrap gap-2 rounded-2xl border border-[#E3E8EF] bg-white p-3">
                <Dropdown
                  ...
                  onChange={setArea}
                  options={areaOptions}
                  triggerClassName="rounded-full border border-[#E3E8EF] px-3 py-2 text-xs"
                  menuClassName="left-0"
                  ariaLabel="Filter restaurants by area"
                />
                <Dropdown
